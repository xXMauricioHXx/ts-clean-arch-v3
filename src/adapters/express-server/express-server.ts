...
export abstract class ExpressServer {
  protected abstract loadControllers(): Controller[];
  ...
  protected buildRoutes(router: Router): Router {
    this.loadControllers().forEach((controller: Controller) => {
      ...
      controller.routeConfigs.forEach((config: RouteConfig) => {
        ...
        if (hasSchema && !schema) {
          throw new Error(
            `Schema to ${
              (controller as unknown as Function).name
            } is mandatory.`
          );
        }

        const func = this.requestHandle(controller, statusCode);
  ...
  private buildMiddlewares(middlewares: Function[]): RequestHandler[] {
    return middlewares.map((middleware: any) => {
      const instanceMiddleware = new middleware();
